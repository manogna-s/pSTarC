...
class Classifier(nn.Module):
    def __init__(self, args, checkpoint_path=None):
        ...
        # 2) ResNet backbone + bottlenck (last fc as bottleneck)
            ...
            model = models.__dict__[args.arch](pretrained=True)
            model.fc = nn.Linear(model.fc.in_features, args.bottleneck)
            bn = nn.BatchNorm1d(args.bottleneck)
            self.encoder = nn.Sequential(model, bn)
            self._output_dim = args.bottleneck

        self.fc = nn.Linear(self.output_dim, args.class_num)
        self.feature_dim = self._output_dim
